        ...
        try
        {
            ...
            await Task.Run(() =>
            {
                foreach (var item in _cleanItems)
                {
                    ...
                    Dispatcher.Invoke(() =>
                    {
                        TxtProgress.Text = $"正在扫描 ({done + 1}/{total})：{item.Name}";
                        UpdateStatistics();
                    });

                    if (item.SpecialType == "WinSxS") { item.SizeBytes = 0; item.FileCount = 0; done++; continue; }
                    if (item.IsRecycler) { var info = GetRecycleBinInfo(); item.SizeBytes = info.Size; item.FileCount = info.Count; done++; continue; }
        ...
        try
        {
            await Task.Run(() =>
            {
                foreach (var item in selected)
                {
                    ...
                    Dispatcher.Invoke(() =>
                    {
                        TxtProgress.Text = $"正在清理 ({cleanDone + 1}/{cleanTotal})：{item.Name}";
                        UpdateStatistics();
                    });

                if (item.SpecialType == "WinSxS") { CleanWinSxS(); cleanDone++; continue; }
                if (item.IsRecycler) { EmptyRecycleBin(); freedSize += item.SizeBytes; item.SizeBytes = 0; item.FileCount = 0; cleanDone++; continue; }
